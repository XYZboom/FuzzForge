...
class {base}Generator(
    ...
    private fun generateClass(name: String): UirClassDeclaration {{
        ...
        val funcContainer = buildFuncContainer {{
            this.functions.addAll(funcs)
        }}
        ...
        return buildClassDeclaration {{
            ...
            this.functionContainer = funcContainer
        }}
    }}
...
class CppGenVisitor : UirDefaultVisitor<Unit, StringBuilder>() {{
    override fun visitProgram(program: UirProgram, data: StringBuilder) {{
        ...
        data.appendLine()
        program.classContainer?.let {{ cc -> visitClassContainer(cc, data) }}
        if (data.length < 100) {{
    ...
    override fun visitClassContainer(container: UirClassContainer, data: StringBuilder) {{
        for (clazz in container.classes) {{
            visitClassDeclaration(clazz, data)
        }}
    }}
    ...
    override fun visitFuncContainer(container: UirFuncContainer, data: StringBuilder) {{
        for (fn in container.functions) {{
            visitFunctionDeclaration(fn, data)
        }}
    }}
    ...
    override fun visitFunctionDeclaration(fd: UirFunctionDeclaration, data: StringBuilder) {{
        val retType = when (fd.returnType) {{
            ...
            is UirPointerType -> {{ val pt = fd.returnType as UirPointerType; if (pt.pointeeType is UirFundamentalType) (pt.pointeeType as UirFundamentalType).name else "void" }}
            else -> "int"
        }}
...
class {base}Runner(
    ...
    private fun compileWith(compiler: String, code: String): RunResult {{
        ...
        val proc = ProcessBuilder(
            compiler, "-std=c++17", "-O2", "-Wall", "-Wextra", "-o",
    ...
    suspend fun diffTestSingle(seed: Long? = null): Pair<RunResult, RunResult> {{
        ...
        val code = translator.translate(program)
        return Pair(compileWith("g++", code), compileWith("clang++", code))
    }}
...
class RunCommand : CliktCommand(name = "run", help = "Run fuzzing campaign with g++") {{
    ...
    override fun run() {{
        ...
        runBlocking {{
            ...
            echo("Done: $ok/$count compiled OK")
            if (failed.isNotEmpty()) {{
                echo("Failures:")
                for (r in failed.take(5)) {{
                    echo("  exit=${{r.exitCode}}")
                    echo(r.stderr.take(300))
                }}
            }}
        }}
    }}
        ...
        runBlocking {{
            ...
            for ((gcc, clang) in results) {{
                ...
                if (gcc.success && clang.success) {{
                    val gccW = gcc.stderr.lines().count {{ "warning:" in it }}
                    val clangW = clang.stderr.lines().count {{ "warning:" in it }}
                    if (gccW != clangW || gcc.stdout != clang.stdout) {{
                        diffMismatch++
                        echo("DIFF: same=${{gcc.exitCode}} gccW=$gccW clangW=$clangW")
                    }}
                }} else if (gcc.success != clang.success) {{
                    diffMismatch++
                    echo("DIFF: one-pass gcc=${{gcc.success}} clang=${{clang.success}}")
                }}
            }}
            echo("Results: g++ $gccOk/$count OK, clang++ $clangOk/$count OK, diffs=$diffMismatch")
        }}
    }}
}}
